...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { type: 'article', url: canonical, title, description, publishedTime: post.publishedAt.toISOString(), modifiedTime: post.updatedAt.toISOString(), authors: [post.author], images: image ? [{ url: image, alt: imageInfo?.alt || post.title }] : undefined },
  ...
  return (
    <NewsShell>
      ...
      <article className={spaceGrotesk.className} style={{ maxWidth: 780, margin: '0 auto', padding: '42px 28px 88px' }}>
        <Link href={NEWS_ORIGIN} style={{ display: 'inline-flex', color: '#5D00F5', fontSize: '0.86rem', fontWeight: 700, textDecoration: 'none', marginBottom: 34 }}><span aria-hidden="true">{'\u2190\u00A0'}</span>HySky News</Link>
        ...
        {imageInfo && <figure style={{ margin: '0 0 38px' }}>
          ...
          {imageInfo.credit && <figcaption style={{ color: '#817989', fontSize: '0.72rem', lineHeight: 1.5, marginTop: 9 }}>
            ...
            {imageInfo.sourceUrl
              ? <a href={imageInfo.sourceUrl} target="_blank" rel="noopener noreferrer" style={{ color: '#5D00F5' }}>{imageInfo.credit}</a>
              : imageInfo.credit}
            {imageInfo.license && <>, {imageInfo.licenseUrl
              ? <>licensed under <a href={imageInfo.licenseUrl} target="_blank" rel="noopener noreferrer" style={{ color: '#5D00F5' }}>{imageInfo.license}</a></>
              : imageInfo.license}</>}.{imageInfo.modified ? ' Cropped for presentation.' : ''}
          </figcaption>}
        </figure>}
