  ...
  return (
    <NewsShell>
      ...
      <article className={spaceGrotesk.className} style={{ maxWidth: 780, margin: '0 auto', padding: '42px 28px 88px' }}>
        <Link href={NEWS_ORIGIN} style={{ display: 'inline-flex', color: '#5D00F5', fontSize: '0.86rem', fontWeight: 700, textDecoration: 'none', marginBottom: 34 }}>← HySky News</Link>
        <div style={{ color: '#5D00F5', fontSize: '0.72rem', fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 13 }}>Standards Watch</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 8, paddingBottom: 25, marginBottom: 34, borderBottom: '1px solid #ece8f1', fontSize: '0.78rem', color: '#8c8593' }}><span style={{ fontWeight: 700, color: '#4a4450' }}>{post.author}</span><span>·</span><span>{new Date(post.publishedAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>{post.readTimeMinutes && <><span>·</span><span>{post.readTimeMinutes} min read</span></>}</div>
